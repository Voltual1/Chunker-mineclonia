// [file name]: com.hivemc.chunker.conversion.encoding.java.base.reader.JavaColumnReader.java
...
public class JavaColumnReader implements ColumnReader {
    private static final Set<String> UNFINISHED_STATUSES = Set.of(
            ...
            "minecraft:noise", "minecraft:surface", "minecraft:carvers", "minecraft:liquid_carvers"
    );
    ...
    public Task<Void> readColumn(ColumnConversionHandler columnConversionHandler) {
        ...
        if (!columnNBT.contains("xPos") || !columnNBT.contains("zPos")) return Task.async("Skipped Column", TaskWeight.LOW, () -> {});
        ...
        if (converter.shouldProcessHeightMap()) processing.add(Task.asyncConsume("Reading HeightMap", TaskWeight.NORMAL, this::readHeightMap, column));
        if (converter.shouldProcessBiomes()) processing.add(Task.asyncConsume("Reading Biomes", TaskWeight.NORMAL, this::readBiomes, column));
    ...
    protected void readLightPopulated(ChunkerColumn column) {
        if (columnNBT.contains("LightPopulated")) column.setLightPopulated(columnNBT.getByte("LightPopulated") != (byte) 0);
    }
    ...
    protected void readHeightMap(ChunkerColumn column) {
        ...
        for (int i = 0; i < heightMap.length; i++) heightMapOutput[i & 0xF][(i >> 4) & 0xF] = (short) heightMap[i];
    ...
    protected void readBiomes(ChunkerColumn column) {
        ...
        if (biomes instanceof ByteArrayTag byteArrayTag && byteArrayTag.getValue() != null) {
            ...
            for (int i = 0; i < chunkerBiomeArray.length; i++) chunkerBiomeArray[i] = resolvers.readBiome(value[i] & 0xFF, dimension);
    ...
    protected void readEntities(ChunkerColumn column) {
        ...
        if (entities != null) {
            for (CompoundTag entityTag : entities) {
                try { readEntity(column, entityTag); } catch (Exception e) { converter.logNonFatalException(new Exception("Failed to process Entity", e)); }
            }
        }
    }
    ...
    protected void readBlockEntities(ChunkerColumn column) {
        ...
        if (blockEntities != null) {
            for (CompoundTag blockEntityTag : blockEntities) {
                try { readBlockEntity(column, blockEntityTag); } catch (Exception e) { converter.logNonFatalException(new Exception("Failed to process BlockEntity", e)); }
            }
        }
    }
    ...
    protected void readChunks(ChunkerColumn column) {
        ...
        for (CompoundTag section : sections) {
            ...
            byte y;
            if (tag instanceof ByteTag byteYTag) y = byteYTag.getValue();
            else if (tag instanceof IntTag intYTag) y = intYTag.getBoxedValue().byteValue();
            else throw new IllegalArgumentException("Invalid Section Y NBT Tag: " + tag);
